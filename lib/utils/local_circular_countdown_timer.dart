...
class LocalCountdownTextFormat {
...
class LocalCountDownTimer extends StatefulWidget {
  ...
  final LocalCountDownController? controller;
  ...
  */
  ...
  @override
  LocalCountDownTimerState createState() => LocalCountDownTimerState();
}

class LocalCountDownTimerState extends State<LocalCountDownTimer>
    with TickerProviderStateMixin {
  AnimationController? _controller;
  Animation<double>? _countDownAnimation;
  LocalCountDownController? countDownController;
  ...
  String _getTime(Duration duration) {
    // For HH:mm:ss format
    if (widget.textFormat == LocalCountdownTextFormat.HH_MM_SS) {
      return '${duration.inHours.toString().padLeft(2, '0')}:${(duration.inMinutes % 60).toString().padLeft(2, '0')}:${(duration.inSeconds % 60).toString().padLeft(2, '0')}';
    }
    // For mm:ss format
    else if (widget.textFormat == LocalCountdownTextFormat.MM_SS) {
      return '${(duration.inMinutes % 60).toString().padLeft(2, '0')}:${(duration.inSeconds % 60).toString().padLeft(2, '0')}';
    }
    // For ss format
    else if (widget.textFormat == LocalCountdownTextFormat.SS) {
      return (duration.inSeconds).toString().padLeft(2, '0');
    }
    // For s format
    else if (widget.textFormat == LocalCountdownTextFormat.S) {
  ...
  @override
  void initState() {
    countDownController = widget.controller ?? LocalCountDownController();
    super.initState();
...
/// Controls (i.e Start, Pause, Resume, Restart) the Countdown Timer.
class LocalCountDownController {
  LocalCountDownTimerState? _state;
  bool? _isReverse;
